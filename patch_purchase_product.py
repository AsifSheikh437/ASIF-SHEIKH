...
select_pattern = r'<select\s+value=\{row\.productId\}\s+onChange=\{e => handleItemProductChange\(idx, e\.target\.value\)\}\s+className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-medium"\s*>\s*\{products\.map\(p => \(\s*<option key=\{p\.id\} value=\{p\.id\}>\s*\{p\.nameBangla\} \(\{p\.unit\}\)\s*</option>\s*\)\)\}\s*</select>'
